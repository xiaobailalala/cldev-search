package com.cldev.search.cldevsearch.process.common;

import org.apache.commons.csv.CSVFormat;
import org.apache.commons.csv.CSVParser;
import org.apache.commons.csv.CSVPrinter;
import org.apache.commons.csv.CSVRecord;
import org.junit.Test;
import org.springframework.util.ObjectUtils;
import org.springframework.util.StringUtils;
import redis.clients.jedis.Jedis;

import java.io.*;
import java.nio.charset.Charset;
import java.nio.charset.StandardCharsets;
import java.util.*;


/**
 * Copyright © 2018 eSunny Info. Developer Stu. All rights reserved.
 * <p>
 * code is far away from bug with the animal protecting
 * <p>
 * ┏┓　　　┏┓
 * ┏┛┻━━━┛┻┓
 * ┃　　　　　　　┃
 * ┃　　　━　　　┃
 * ┃　┳┛　┗┳　┃
 * ┃　　　　　　　┃
 * ┃　　　┻　　　┃
 * ┃　　　　　　　┃
 * ┗━┓　　　┏━┛
 * 　　┃　　　┃神兽保佑
 * 　　┃　　　┃代码无BUG！
 * 　　┃　　　┗━━━┓
 * 　　┃　　　　　　　┣┓
 * 　　┃　　　　　　　┏┛
 * 　　┗┓┓┏━┳┓┏┛
 * 　　　┃┫┫　┃┫┫
 * 　　　┗┻┛　┗┻┛
 *
 * @author zpx
 * Build File @date: 2019/10/2 3:41 下午
 * @version 1.0
 * @description
 */
public class FileTest {

    @Test
    public void labelOperator() {
        File labelFile = new File("C:\\Users\\cl24\\Desktop\\allBlogfiled(1).csv");
        try (InputStream inputStream = new FileInputStream(labelFile);
             CSVParser csvRecords = new CSVParser(new InputStreamReader(inputStream, StandardCharsets.UTF_8), CSVFormat.DEFAULT.withHeader("uid", "label")
                     .withSkipHeaderRecord(true))) {
            Set<String> labelSet = new LinkedHashSet<>();
            for (CSVRecord item : csvRecords) {
                String labelStr = item.get("label");
                if (!StringUtils.isEmpty(labelStr)) {
                    for (String label : labelStr.split("_")) {
                        if (!StringUtils.isEmpty(label) && !ObjectUtils.isEmpty(label)) {
                            labelSet.add(label);
                        }
                    }
                }
            }
            File file = new File("label-mapping-02");
            FileOutputStream fileOutputStream = null;
            try {
                fileOutputStream = new FileOutputStream(file, true);
                int index = 0;
                for (String s : labelSet) {
                    byte[] contentInBytes = (s + " " + (index++) + "\r,").getBytes();
                    fileOutputStream.write(contentInBytes);
                }
                fileOutputStream.flush();
                fileOutputStream.close();
            } catch (IOException e) {
                e.printStackTrace();
            } finally {
                try {
                    if (fileOutputStream != null) {
                        fileOutputStream.close();
                    }
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }

            System.out.println(labelSet.size());
        } catch (IOException ignored) {
        }
    }

    @Test
    public void newsOperator() {
        File csvFile = new File("C:\\Users\\cl24\\Desktop\\uid_name_frequency_gte_100_blue.csv");
        try (InputStream inputStream = new FileInputStream(csvFile);
             CSVParser csvRecords = new CSVParser(new InputStreamReader(inputStream, StandardCharsets.UTF_8),
                     CSVFormat.DEFAULT.withHeader("uid", "name", "frequency")
                             .withSkipHeaderRecord(false))) {
            int count = 0;
            for (CSVRecord record : csvRecords.getRecords()) {
                String name = record.get("name");
                if (name.contains("报") || name.contains("刊") || name.contains("新闻") || name.contains("国") || name.contains("财经") || name.contains("邪教")) {
                    printLoadLog("newsMedia", record.get("uid") + "      " + name + "      " + record.get("frequency"));
                    count++;
                }
            }
            System.out.println(count);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void csvParse() {
        File csvFile = new File("C:\\Users\\cl24\\Desktop\\kol无效博文百分比.csv");
        try (InputStream inputStream = new FileInputStream(csvFile);
             CSVParser csvRecords = new CSVParser(new InputStreamReader(inputStream, StandardCharsets.UTF_8),
                     CSVFormat.DEFAULT.withHeader("uid", "count")
                             .withSkipHeaderRecord(true))) {
            for (CSVRecord record : csvRecords.getRecords()) {
                printLoadLog("online-water-amy", record.get("uid") + "  " + record.get("count"));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

    @Test
    public void readCsv() {
        File csvFile = new File("C:\\Users\\cl24\\Desktop\\allBlog_sum_res(3).csv");
        try (InputStream inputStream = new FileInputStream(csvFile);
             CSVParser csvRecords = new CSVParser(new InputStreamReader(inputStream, StandardCharsets.UTF_8), CSVFormat.DEFAULT.withHeader("uid", "label")
                     .withSkipHeaderRecord(false))) {
            Set<String> label = new LinkedHashSet<>();
            for (CSVRecord record : csvRecords) {
                String labelStr = record.get("label");
                if (!StringUtils.isEmpty(labelStr)) {
                    for (String item : labelStr.split("\\|")) {
                        if (!ObjectUtils.isEmpty(item) && !StringUtils.isEmpty(item)) {
                            label.add(item.split(":")[0]);
                        }
                    }
                }
            }
            System.out.println(label.size());
            System.out.println(label);
        } catch (IOException ignored) {
        }
    }

    @Test
    public void searchLabelFromRedis() {
        Jedis jedis = new Jedis("192.168.2.11", 6399);
        jedis.auth("cldev");
        String item = jedis.get("6226234826");
        System.out.println(item);
    }

    @Test
    public void insertRedis() throws IOException {
        Jedis jedis = new Jedis("192.168.2.11", 6399);
        jedis.auth("cldev");
        File mappingFile = new File("config\\label-mapping-02");
        BufferedReader reader = new BufferedReader(new FileReader(mappingFile));
        String tempString;
        Map<String, String> mapping = new HashMap<>(60);
        while ((tempString = reader.readLine()) != null) {
            String[] str = tempString.split(" ");
            mapping.put(str[0], str[1]);
        }
        reader.close();
        try (InputStream inputStream = new FileInputStream(new File("C:\\Users\\cl24\\Desktop\\allBlog_sum_res(3).csv"));
             CSVParser csvRecords = new CSVParser(new InputStreamReader(inputStream, StandardCharsets.UTF_8), CSVFormat.DEFAULT.withHeader("uid", "label")
                     .withSkipHeaderRecord(false))) {
            for (CSVRecord record : csvRecords) {
                String labelStr = record.get("label");
                Set<String> labelSet = new HashSet<>();
                if (!StringUtils.isEmpty(labelStr)) {
                    for (String label : labelStr.split("\\|")) {
                        if (!StringUtils.isEmpty(label) && !ObjectUtils.isEmpty(label)) {
                            labelSet.add(label);
                        }
                    }
                }
                StringBuilder result = new StringBuilder();
                for (String item : labelSet) {
                    String[] label = item.split(":");
                    result.append(mapping.get(label[0])).append("-").append(label[1]).append(",");
                }
                String res = result.length() == 0 ? "" : result.substring(0, result.length() - 1);
                jedis.set(record.get("uid"), res);
            }
        }
        Jedis jedisWithoutScore = new Jedis("192.168.2.11", 6409);
        jedisWithoutScore.auth("cldev");
        try (InputStream inputStream = new FileInputStream(new File("C:\\Users\\cl24\\Desktop\\allBlogfiled(1).csv"));
             CSVParser csvRecords = new CSVParser(new InputStreamReader(inputStream, StandardCharsets.UTF_8), CSVFormat.DEFAULT.withHeader("uid", "label")
                     .withSkipHeaderRecord(true))) {
            for (CSVRecord record : csvRecords) {
                String labelStr = record.get("label");
                Set<String> labelSet = new HashSet<>();
                if (!StringUtils.isEmpty(labelStr)) {
                    for (String label : labelStr.split("_")) {
                        if (!StringUtils.isEmpty(label) && !ObjectUtils.isEmpty(label)) {
                            labelSet.add(label);
                        }
                    }
                }
                StringBuilder result = new StringBuilder();
                for (String item : labelSet) {
                    result.append(mapping.get(item)).append("-");
                }
                String res = result.length() == 0 ? "" : result.substring(0, result.length() - 1);
                jedisWithoutScore.set(record.get("uid"), res);
            }
        }
    }

    @Test
    public void csvWrite() throws IOException {
        String name = "何炅@@@@@@@@何老师," +
                "杨幂@@@@@@@@杨幂," +
                "宁泽涛@@@@@@@@宁泽涛," +
                "张亮Sean@@@@@@@@张亮," +
                "羽泉@@@@@@@@羽泉," +
                "沙溢@@@@@@@@沙溢," +
                "熊梓淇@@@@@@@@熊梓淇," +
                "蔡明@@@@@@@@蔡明," +
                "CoCo李玟@@@@@@@@李玟," +
                "孙红雷@@@@@@@@孙红雷," +
                "潘瑋柏@@@@@@@@潘玮柏," +
                "容祖儿@@@@@@@@容祖儿," +
                "刘同@@@@@@@@刘同," +
                "韩东君ElvisHan@@@@@@@@韩东君," +
                "王杰@@@@@@@@王杰," +
                "R1SE-焉栩嘉@@@@@@@@焉栩嘉," +
                "景甜@@@@@@@@景甜," +
                "章子怡@@@@@@@@章子怡," +
                "江疏影@@@@@@@@江疏影," +
                "蔡健雅@@@@@@@@蔡健雅," +
                "向佐JackyHeung@@@@@@@@向佐," +
                "金秀贤@@@@@@@@金秀贤," +
                "王栎鑫@@@@@@@@王栎鑫," +
                "高海寧@@@@@@@@海宁," +
                "肖央@@@@@@@@肖央," +
                "墨香铜臭MXTX@@@@@@@@墨香铜臭," +
                "杨宗纬@@@@@@@@杨宗纬," +
                "张力-飞视设计@@@@@@@@张力," +
                "王博文小白@@@@@@@@王博文," +
                "乔振宇@@@@@@@@乔振宇," +
                "陈都灵@@@@@@@@陈都灵," +
                "王子异@@@@@@@@王子异," +
                "白岩松-wys@@@@@@@@白岩松," +
                "东野圭吾饭团组@@@@@@@@东野圭吾," +
                "冯提莫@@@@@@@@冯提莫," +
                "张国荣微吧@@@@@@@@张国荣," +
                "希天才@@@@@@@@金希澈," +
                "郎平@@@@@@@@郎平," +
                "于晓光@@@@@@@@于晓光," +
                "徐璐LULU@@@@@@@@徐璐," +
                "朱军99@@@@@@@@朱军," +
                "BIGBIG张大大@@@@@@@@张大大," +
                "秦岭二月@@@@@@@@秦岭," +
                "我是娜扎@@@@@@@@古力娜扎," +
                "秦海璐@@@@@@@@秦海璐," +
                "尚雯婕@@@@@@@@尚雯婕," +
                "于正1978@@@@@@@@于正," +
                "宫崎骏漫画全集@@@@@@@@宫崎骏," +
                "杜江侦察记@@@@@@@@杜江," +
                "马良@@@@@@@@马良," +
                "林家狮哥@@@@@@@@林家," +
                "坏猴子宁浩@@@@@@@@宁浩," +
                "夢想家林志穎@@@@@@@@林志颖," +
                "蕭亞軒MISSELVA@@@@@@@@萧亚轩," +
                "白宇WHITE@@@@@@@@白宇," +
                "王子文Olivia@@@@@@@@王子文," +
                "卜凡KATTO@@@@@@@@卜凡," +
                "高晓松@@@@@@@@高晓松," +
                "舒畅@@@@@@@@舒畅," +
                "LIMYOONA90@@@@@@@@林允儿," +
                "林忆莲@@@@@@@@林忆莲," +
                "陈志朋@@@@@@@@陈志朋," +
                "王磊ST0NE@@@@@@@@王磊," +
                "张嘉倪@@@@@@@@张嘉倪," +
                "楊千嬅@@@@@@@@杨千嬅," +
                "李冰冰@@@@@@@@李冰冰," +
                "刘谦@@@@@@@@刘谦," +
                "高发的微博@@@@@@@@高发," +
                "王传君@@@@@@@@王传君," +
                "莫言@@@@@@@@莫言," +
                "张爱玲@@@@@@@@张爱玲," +
                "安宰贤0701@@@@@@@@安宰贤," +
                "火箭少女101官博@@@@@@@@火箭少女101," +
                "演员马可@@@@@@@@马可," +
                "陈欧@@@@@@@@陈欧," +
                "大张伟@@@@@@@@张伟," +
                "TFBOYS-王俊凯@@@@@@@@王俊凯," +
                "黄圣依@@@@@@@@黄圣依," +
                "五月天@@@@@@@@五月天," +
                "倾听秋雨@@@@@@@@余秋雨," +
                "王宝强@@@@@@@@王宝强," +
                "董洁@@@@@@@@董洁," +
                "Sunlight杨芸晴个人站@@@@@@@@杨芸晴," +
                "伊能靜@@@@@@@@伊能静," +
                "王尼玛@@@@@@@@王尼玛," +
                "李艺彤@@@@@@@@李艺彤," +
                "金耳朵兔子@@@@@@@@马蓉," +
                "高桥冯巩@@@@@@@@冯巩," +
                "captain龙@@@@@@@@马龙," +
                "潘粤明@@@@@@@@潘粤明," +
                "孟小发的生活@@@@@@@@孟非," +
                "何炅@@@@@@@@何炅," +
                "黄旭BooM@@@@@@@@黄旭," +
                "凤凰传奇@@@@@@@@凤凰传奇," +
                "米娜@@@@@@@@米娜," +
                "朱广权@@@@@@@@朱广权," +
                "周知@@@@@@@@周知," +
                "张云龙mmc@@@@@@@@张云龙," +
                "吉克隽逸@@@@@@@@吉克隽逸," +
                "Karen莫文蔚@@@@@@@@莫文蔚," +
                "素描闫妮@@@@@@@@闫妮," +
                "张彬彬Vin@@@@@@@@张彬彬," +
                "李兰迪@@@@@@@@李兰迪," +
                "廖凡@@@@@@@@廖凡," +
                "京剧演员张凯@@@@@@@@张凯," +
                "黄渤@@@@@@@@黄渤," +
                "邓超@@@@@@@@邓超," +
                "大连食客高@@@@@@@@高强," +
                "张国立@@@@@@@@张国立," +
                "钱钟书日记@@@@@@@@钱钟书," +
                "林丹@@@@@@@@林丹," +
                "刘涛tamia@@@@@@@@刘涛," +
                "孟鹤堂@@@@@@@@孟鹤堂," +
                "郑云龙DL@@@@@@@@郑云龙," +
                "林更新@@@@@@@@林更新," +
                "angelababy@@@@@@@@杨颖," +
                "钟丽缇Christy@@@@@@@@钟丽缇," +
                "陈学冬@@@@@@@@陈学冬," +
                "陈思诚@@@@@@@@陈思诚," +
                "吳青峰@@@@@@@@吴青峰," +
                "刘烨@@@@@@@@刘烨," +
                "金在中_Official@@@@@@@@金在中," +
                "胡歌@@@@@@@@胡歌," +
                "張鈞甯@@@@@@@@张钧甯," +
                "吴尊@@@@@@@@吴尊," +
                "陈晓@@@@@@@@陈晓," +
                "周琦_9@@@@@@@@周琦," +
                "张学良@@@@@@@@张学良," +
                "王大陸TaluWang@@@@@@@@王大陆," +
                "Baidu金泰亨吧@@@@@@@@金泰亨," +
                "傅园慧o_O@@@@@@@@傅园慧," +
                "Ming奚梦瑶@@@@@@@@奚梦瑶," +
                "纪凌尘chan@@@@@@@@纪凌尘," +
                "宋祖儿lareina@@@@@@@@宋祖儿," +
                "UNIQ-王一博@@@@@@@@王一博," +
                "汪洋@@@@@@@@汪洋," +
                "飞轮海中文网@@@@@@@@飞轮海," +
                "白敬亭@@@@@@@@白敬亭," +
                "徐海乔@@@@@@@@徐海乔," +
                "刘鑫6_6@@@@@@@@刘鑫," +
                "帅仔_周张弛@@@@@@@@张弛," +
                "王祖藍@@@@@@@@王祖蓝," +
                "江小爬LOVE@@@@@@@@江一燕," +
                "赵丽颖@@@@@@@@赵丽颖," +
                "艾福杰尼@@@@@@@@艾福杰尼," +
                "陈翔橙@@@@@@@@陈翔," +
                "罗晋@@@@@@@@罗晋," +
                "木子洋KWIN@@@@@@@@木子洋," +
                "石原里美闲聊会@@@@@@@@石原里美," +
                "盛一伦@@@@@@@@盛一伦," +
                "倪妮V@@@@@@@@倪妮," +
                "姚贝娜@@@@@@@@姚贝娜," +
                "张涵予@@@@@@@@张涵予," +
                "钟楚曦@@@@@@@@钟楚曦," +
                "wajijiwa@@@@@@@@龙丹妮," +
                "郭俊辰_Fiction@@@@@@@@郭俊辰," +
                "郭京飞@@@@@@@@郭京飞," +
                "丁宁@@@@@@@@丁宁," +
                "刘恺威@@@@@@@@刘恺威," +
                "许巍音乐朋友@@@@@@@@许巍," +
                "孙坚@@@@@@@@孙坚," +
                "方文山@@@@@@@@方文山," +
                "李赫宰吧SmileHyuk@@@@@@@@李赫宰," +
                "郑恺@@@@@@@@郑恺," +
                "黄晓明@@@@@@@@黄晓明," +
                "周一围@@@@@@@@周一围," +
                "Dear-迪丽热巴@@@@@@@@迪丽热巴," +
                "张柏芝@@@@@@@@张柏芝," +
                "陳奕迅所長@@@@@@@@陈奕迅," +
                "邓丽君我姓唐@@@@@@@@邓丽君," +
                "肖邦的诗@@@@@@@@肖邦," +
                "GDRAGON_OFFICIAL@@@@@@@@权志龙," +
                "陈乔恩@@@@@@@@陈乔恩," +
                "辛芷蕾@@@@@@@@辛芷蕾," +
                "主持人董卿的那些事@@@@@@@@董卿," +
                "Baidu_闵玧其Suga吧@@@@@@@@闵玧其," +
                "杜淳@@@@@@@@杜淳," +
                "陈芊芊Amelie@@@@@@@@芊芊," +
                "丹尼@@@@@@@@丹尼," +
                "戚薇@@@@@@@@戚薇," +
                "橋本環奈后援站@@@@@@@@桥本环奈," +
                "李玉刚@@@@@@@@李玉刚," +
                "海陆@@@@@@@@海陆," +
                "吴迪多寂寞@@@@@@@@吴迪," +
                "周冬雨@@@@@@@@周冬雨," +
                "李宇春@@@@@@@@李宇春," +
                "余光中@@@@@@@@余光中," +
                "蒂姆嘟嘟Tim@@@@@@@@蒂姆," +
                "巩俐666@@@@@@@@巩俐," +
                "乔欣Bridgette@@@@@@@@乔欣," +
                "大壮爱唱歌@@@@@@@@大壮," +
                "沈腾@@@@@@@@沈腾," +
                "哈囉李敖@@@@@@@@李敖," +
                "鄭元暢小綜@@@@@@@@郑元畅," +
                "吴谨言@@@@@@@@吴谨言," +
                "李咏@@@@@@@@李咏," +
                "大鹏董成鹏@@@@@@@@大鹏," +
                "吴刚de微博@@@@@@@@吴刚," +
                "林志玲@@@@@@@@林志玲," +
                "Kelly于文文@@@@@@@@于文文," +
                "张铭恩_@@@@@@@@张铭恩," +
                "宇宙少女_程潇@@@@@@@@程潇," +
                "姜潮@@@@@@@@姜潮," +
                "姜文电影官微@@@@@@@@姜文," +
                "王石@@@@@@@@王石," +
                "古龙@@@@@@@@古龙," +
                "李飞@@@@@@@@李飞," +
                "曹云金@@@@@@@@曹云金," +
                "大张伟@@@@@@@@大张伟," +
                "张晋max@@@@@@@@张晋," +
                "歐陽娜娜Nana@@@@@@@@欧阳娜娜," +
                "邱澤日記@@@@@@@@邱泽," +
                "王炸炸wz@@@@@@@@王炸," +
                "主播李湘@@@@@@@@李湘," +
                "X玖少年团伍嘉成@@@@@@@@伍嘉成," +
                "小黑子LLLLLOVE@@@@@@@@黑子," +
                "毕淑敏@@@@@@@@毕淑敏," +
                "TF家族-敖子逸@@@@@@@@敖子逸," +
                "费玉清贴吧@@@@@@@@费玉清," +
                "Leah童Dou@@@@@@@@窦靖童," +
                "罗永浩@@@@@@@@罗永浩," +
                "羅志祥@@@@@@@@罗志祥," +
                "冯唐@@@@@@@@冯唐," +
                "徐佳瑩@@@@@@@@徐佳莹," +
                "蔡少芬@@@@@@@@蔡少芬," +
                "曾轶可NewestNews@@@@@@@@曾轶可," +
                "火箭少女101_傅菁@@@@@@@@傅菁," +
                "张雪迎Sophie@@@@@@@@张雪迎," +
                "邢昭林@@@@@@@@邢昭林," +
                "MD_摩登兄弟@@@@@@@@刘宇宁," +
                "张扬张杨@@@@@@@@张扬," +
                "TFBOYS-易烊千玺@@@@@@@@TFBOYS," +
                "我谁也不是呢啊@@@@@@@@许可," +
                "汤唯--@@@@@@@@汤唯," +
                "白百何@@@@@@@@白百何," +
                "沈梦辰@@@@@@@@沈梦辰," +
                "張學友歌迷會@@@@@@@@张学友," +
                "UNINE_管栎@@@@@@@@管栎," +
                "李斌@@@@@@@@李斌," +
                "深情的高伟光@@@@@@@@高伟光," +
                "李光洁@@@@@@@@李光洁," +
                "邓伦@@@@@@@@邓伦," +
                "钟汉良@@@@@@@@钟汉良," +
                "彭于晏@@@@@@@@彭于晏," +
                "一号立井@@@@@@@@李亚鹏," +
                "翟天临@@@@@@@@翟天临," +
                "王祖賢官方微博@@@@@@@@王祖贤," +
                "靳东@@@@@@@@靳东," +
                "高飞@@@@@@@@高飞," +
                "杨烁@@@@@@@@杨烁," +
                "林俊杰@@@@@@@@林俊杰," +
                "撒贝宁@@@@@@@@撒贝宁," +
                "上海安徒生童话乐园@@@@@@@@安徒生," +
                "雪梨Cherie@@@@@@@@雪梨," +
                "朱星杰J_zen@@@@@@@@朱星杰," +
                "高明Hero@@@@@@@@高明," +
                "李宗盛bot@@@@@@@@李宗盛," +
                "陈凯歌@@@@@@@@陈凯歌," +
                "R1SE-夏之光@@@@@@@@夏之光," +
                "华杰工作室@@@@@@@@霍建华," +
                "火箭少女101_孟美岐@@@@@@@@孟美岐," +
                "李银河@@@@@@@@李银河," +
                "李光洙@@@@@@@@李光洙," +
                "陆毅@@@@@@@@陆毅," +
                "黄文-2011@@@@@@@@黄文," +
                "张翰@@@@@@@@张翰," +
                "贾静雯@@@@@@@@贾静雯," +
                "黄轩的微博@@@@@@@@黄轩," +
                "汪苏泷@@@@@@@@汪苏泷," +
                "李连杰@@@@@@@@李连杰," +
                "Happy张江@@@@@@@@张江," +
                "蒋欣@@@@@@@@蒋欣," +
                "小陶虹@@@@@@@@陶虹," +
                "周密主编@@@@@@@@周密," +
                "包贝尔@@@@@@@@包贝尔," +
                "张子枫@@@@@@@@张子枫," +
                "柴静看见@@@@@@@@柴静," +
                "张丹峰@@@@@@@@张丹峰," +
                "李子璇豆子@@@@@@@@李子璇," +
                "林清玄@@@@@@@@林清玄," +
                "杨洋@@@@@@@@杨洋," +
                "高峰422@@@@@@@@高峰," +
                "蔡徐坤@@@@@@@@蔡徐坤," +
                "龙应台@@@@@@@@龙应台," +
                "许魏洲ZZ@@@@@@@@许魏洲," +
                "吳京@@@@@@@@吴京," +
                "张小娴@@@@@@@@张小娴," +
                "UNINE_姚明明@@@@@@@@姚明," +
                "Avril_Lavigne@@@@@@@@艾薇儿," +
                "余文樂@@@@@@@@余文乐," +
                "李泽言K@@@@@@@@李泽," +
                "朴敏英@@@@@@@@朴敏英," +
                "庾澄慶@@@@@@@@庾澄庆," +
                "焦俊艳@@@@@@@@焦俊艳," +
                "林夕@@@@@@@@林夕," +
                "火箭少女101_段奥娟@@@@@@@@段奥娟," +
                "赵本山@@@@@@@@赵本山," +
                "李雪515@@@@@@@@李雪," +
                "荣斋燕居@@@@@@@@沈从文," +
                "火箭少女101_吴宣仪@@@@@@@@吴宣仪," +
                "袁泉@@@@@@@@袁泉," +
                "都暻秀吧DohkyungsooBar@@@@@@@@都暻秀," +
                "宋丹丹@@@@@@@@宋丹丹," +
                "Jimin朴智旻Bar@@@@@@@@朴智旻," +
                "佟大为@@@@@@@@佟大为," +
                "壳内张曼玉@@@@@@@@张曼玉," +
                "朱亚文@@@@@@@@朱亚文," +
                "张一山@@@@@@@@张一山," +
                "张靓颖@@@@@@@@张靓颖," +
                "黄宗泽@@@@@@@@黄宗泽," +
                "郭采洁@@@@@@@@郭采洁," +
                "佘詩曼Charmaine@@@@@@@@佘诗曼," +
                "郭靖@@@@@@@@郭靖," +
                "wu世勋-EXO@@@@@@@@吴世勋," +
                "彭昱畅@@@@@@@@彭昱畅," +
                "明道@@@@@@@@明道," +
                "陈立农@@@@@@@@陈立农," +
                "法国马赛旅游会议局@@@@@@@@马赛," +
                "林峯@@@@@@@@林峯," +
                "Che_nn@@@@@@@@王晨," +
                "宋佳@@@@@@@@宋佳," +
                "冯小刚@@@@@@@@冯小刚," +
                "SNH48-黄婷婷@@@@@@@@黄婷婷," +
                "刘诗诗@@@@@@@@刘诗诗," +
                "梁静茹@@@@@@@@梁静茹," +
                "歌手李健@@@@@@@@李健," +
                "任嘉伦Allen@@@@@@@@任嘉伦," +
                "張智霖@@@@@@@@张智霖," +
                "温子仁@@@@@@@@温子仁," +
                "HannahQuinlivan@@@@@@@@昆凌," +
                "周锐Zee@@@@@@@@周锐," +
                "时代少年团队长-马嘉祺@@@@@@@@马嘉祺," +
                "霍思燕@@@@@@@@霍思燕," +
                "詹姆斯吧官方微博@@@@@@@@詹姆斯," +
                "贾乃亮@@@@@@@@贾乃亮," +
                "腾格尔@@@@@@@@腾格尔," +
                "菅田将晖_SUDA@@@@@@@@菅田将晖," +
                "具惠善中文网微博@@@@@@@@具惠善," +
                "董明珠自媒体@@@@@@@@董明珠," +
                "贾跃亭@@@@@@@@贾跃亭," +
                "周立波@@@@@@@@周立波," +
                "杨坤@@@@@@@@杨坤," +
                "王嘉Jevon@@@@@@@@王嘉," +
                "汪东城@@@@@@@@汪东城," +
                "_董大力@@@@@@@@董力," +
                "劉嘉玲@@@@@@@@刘嘉玲," +
                "井柏然@@@@@@@@井柏然," +
                "陳柏霖@@@@@@@@陈柏霖," +
                "黄致列HCY@@@@@@@@黄致列," +
                "羽生结弦资讯台@@@@@@@@羽生结弦," +
                "王嘉尔@@@@@@@@王嘉尔," +
                "林海音乐@@@@@@@@林海," +
                "张杰@@@@@@@@张杰," +
                "泫雅_hyuna@@@@@@@@泫雅," +
                "杨柳@@@@@@@@杨柳," +
                "杨紫@@@@@@@@杨紫," +
                "张一山@@@@@@@@刘星," +
                "刘宪华Henry-Lau@@@@@@@@刘宪华," +
                "杜海涛@@@@@@@@杜海涛," +
                "钱枫oscarqian@@@@@@@@钱枫," +
                "王伟@@@@@@@@王伟," +
                "陈粒_@@@@@@@@陈粒," +
                "张家辉@@@@@@@@张家辉," +
                "郭德纲@@@@@@@@郭德纲," +
                "张远Bird@@@@@@@@张远," +
                "桑格格桑格格@@@@@@@@格格," +
                "王彦霖@@@@@@@@王彦霖," +
                "金晨@@@@@@@@金晨," +
                "张碧晨@@@@@@@@张碧晨," +
                "魏大勋@@@@@@@@魏大勋," +
                "张嘉佳@@@@@@@@张嘉佳," +
                "马上又@@@@@@@@马上又," +
                "刘国梁@@@@@@@@刘国梁," +
                "YH宇宙少女@@@@@@@@宇宙少女," +
                "周洁琼_OFFICIAL@@@@@@@@周洁琼," +
                "馬天宇@@@@@@@@马天宇," +
                "贾樟柯@@@@@@@@贾樟柯," +
                "UNINE_李汶翰@@@@@@@@李汶翰," +
                "松原发布@@@@@@@@松原," +
                "蘇有朋@@@@@@@@苏有朋," +
                "Biiiii毕雯珺@@@@@@@@毕雯珺," +
                "秦奋King@@@@@@@@秦奋," +
                "乐华娱乐杜华@@@@@@@@杜华," +
                "THEO-朱正廷@@@@@@@@朱正廷," +
                "惠英紅kara@@@@@@@@惠英红," +
                "aMEI_feat_阿密特@@@@@@@@张惠妹," +
                "李彦宏@@@@@@@@李彦宏," +
                "谢娜@@@@@@@@谢娜," +
                "佟丽娅@@@@@@@@佟丽娅," +
                "海清@@@@@@@@海清," +
                "王甜甜@@@@@@@@王甜甜," +
                "应采儿@@@@@@@@应采儿," +
                "单田芳@@@@@@@@单田芳," +
                "刘昊然turbo@@@@@@@@刘昊然," +
                "许凯soso@@@@@@@@许凯," +
                "岳云鹏@@@@@@@@岳云鹏," +
                "周华健@@@@@@@@周华健," +
                "Chic韩风@@@@@@@@韩风," +
                "刘翔-soulmate@@@@@@@@刘翔," +
                "袁弘@@@@@@@@袁弘," +
                "TFBOYS-易烊千玺@@@@@@@@易烊千玺," +
                "李振-Lee@@@@@@@@李振," +
                "范冰冰@@@@@@@@范冰冰," +
                "周天@@@@@@@@周天," +
                "TIA袁娅维@@@@@@@@袁娅维," +
                "田亮@@@@@@@@田亮," +
                "八家户村@@@@@@@@陈建斌," +
                "许志安@@@@@@@@许志安," +
                "柳岩@@@@@@@@柳岩," +
                "张新成@@@@@@@@张新成," +
                "董冬咚d@@@@@@@@董子健," +
                "蓝洁瑛影迷会@@@@@@@@蓝洁瑛," +
                "李小璐Super璐@@@@@@@@李小璐," +
                "edc陳冠希@@@@@@@@陈冠希," +
                "于朦胧@@@@@@@@于朦胧," +
                "蓝盈莹Lyric@@@@@@@@蓝盈莹," +
                "范范范瑋琪@@@@@@@@范玮琪," +
                "不剪刘海的狗妹@@@@@@@@刘海," +
                "Lee权哲@@@@@@@@李权哲," +
                "谭维维@@@@@@@@谭维维," +
                "许嵩@@@@@@@@许嵩," +
                "Johnny黄景瑜@@@@@@@@黄景瑜," +
                "张嘉译工作室@@@@@@@@张嘉译," +
                "高亮_萝卜北美@@@@@@@@高亮," +
                "俞飞鸿工作室@@@@@@@@俞飞鸿," +
                "Tien张天@@@@@@@@张天," +
                "李沁@@@@@@@@李沁," +
                "R1SE-周震南@@@@@@@@周震南," +
                "李娜@@@@@@@@李娜," +
                "万茜@@@@@@@@万茜," +
                "UNINE_嘉羿@@@@@@@@嘉羿," +
                "王珞丹@@@@@@@@王珞丹," +
                "張軒睿Derek@@@@@@@@张轩睿," +
                "R1SE-赵磊@@@@@@@@赵磊," +
                "夏雨@@@@@@@@夏雨," +
                "海明威评@@@@@@@@海明威," +
                "蔡康永@@@@@@@@蔡康永," +
                "歐陽靖MCJin@@@@@@@@欧阳靖," +
                "Crystal张天爱@@@@@@@@张天爱," +
                "陈羽凡@@@@@@@@陈羽凡," +
                "东方神起@@@@@@@@东方神起," +
                "林宥嘉YogaLin@@@@@@@@林宥嘉," +
                "VanNess吳建豪@@@@@@@@吴建豪," +
                "杨蓉@@@@@@@@杨蓉," +
                "Rihanna_official@@@@@@@@蕾哈娜," +
                "演员王迅@@@@@@@@王迅," +
                "梁博--@@@@@@@@梁博," +
                "曾舜晞Joseph@@@@@@@@曾舜晞," +
                "JeremyLin林书豪@@@@@@@@林书豪," +
                "理查德RichardsG@@@@@@@@理查德," +
                "花泽香菜@@@@@@@@花泽香菜," +
                "段奕宏@@@@@@@@段奕宏," +
                "田馥甄Hebe@@@@@@@@田馥甄," +
                "吴磊LEO@@@@@@@@吴磊," +
                "楊丞琳@@@@@@@@杨丞琳," +
                "宋茜@@@@@@@@宋茜," +
                "KRYSTALJUNG@@@@@@@@郑秀晶," +
                "那英@@@@@@@@那英," +
                "林允Jelly@@@@@@@@林允," +
                "刘雯@@@@@@@@刘雯," +
                "秦俊杰@@@@@@@@秦俊杰," +
                "马思纯@@@@@@@@马思纯," +
                "黄毅清@@@@@@@@黄毅清," +
                "言承旭Jerry@@@@@@@@言承旭," +
                "孙杨@@@@@@@@孙杨," +
                "张卫健@@@@@@@@张卫健," +
                "张若昀@@@@@@@@张若昀," +
                "Tennis张帅@@@@@@@@张帅," +
                "努力努力再努力x@@@@@@@@张艺兴," +
                "林依晨Ariel@@@@@@@@林依晨," +
                "娄艺潇@@@@@@@@娄艺潇," +
                "洪欣@@@@@@@@洪欣," +
                "Mr_凡先生@@@@@@@@吴亦凡," +
                "马伯骞Victor@@@@@@@@马伯骞," +
                "高云翔@@@@@@@@高云翔," +
                "秦昊@@@@@@@@秦昊," +
                "周全不周全@@@@@@@@周全," +
                "精彩高迪@@@@@@@@高迪," +
                "孙怡微博@@@@@@@@孙怡," +
                "古巨基LeoKu@@@@@@@@古巨基," +
                "胡一天啊@@@@@@@@胡一天," +
                "小栗旬@@@@@@@@小栗旬," +
                "周笔畅@@@@@@@@周笔畅," +
                "潘长江@@@@@@@@潘长江," +
                "刘强东@@@@@@@@刘强东," +
                "陳綺貞cheerego@@@@@@@@陈绮贞," +
                "于小彤@@@@@@@@于小彤," +
                "秦汉@@@@@@@@秦汉," +
                "郭麒麟@@@@@@@@郭麒麟," +
                "我是涂磊@@@@@@@@涂磊," +
                "亦舒@@@@@@@@亦舒," +
                "赵雷Z@@@@@@@@赵雷," +
                "演员陶醉@@@@@@@@陶醉," +
                "刘若英@@@@@@@@刘若英," +
                "毛晓彤@@@@@@@@毛晓彤," +
                "鎮宇DD的豬頭俱樂部@@@@@@@@吴镇宇," +
                "王小波语录@@@@@@@@王小波," +
                "jjjustin0219@@@@@@@@黄明昊," +
                "袁姗姗@@@@@@@@袁姗姗," +
                "張信哲JeffChang@@@@@@@@张信哲," +
                "火箭少女101_杨超越@@@@@@@@杨超越," +
                "武艺@@@@@@@@武艺," +
                "real__pcyyyyy@@@@@@@@朴灿烈," +
                "乡村教师代言人-马云@@@@@@@@马云," +
                "LeonKysonLee李安@@@@@@@@李安," +
                "吴秀波@@@@@@@@吴秀波," +
                "女排朱婷ZT@@@@@@@@朱婷," +
                "演员艾伦Allen@@@@@@@@艾伦," +
                "胡夏@@@@@@@@胡夏," +
                "陳意涵@@@@@@@@陈意涵," +
                "成龙@@@@@@@@成龙," +
                "金瀚Kim@@@@@@@@金瀚," +
                "尹正@@@@@@@@尹正," +
                "齐衡小甜豆@@@@@@@@齐衡," +
                "安德鲁过马路@@@@@@@@安德鲁," +
                "M鹿M@@@@@@@@鹿晗," +
                "张馨予@@@@@@@@张馨予," +
                "车银优吧_ChaEunWooBar@@@@@@@@车银优," +
                "魏晨@@@@@@@@魏晨," +
                "文章同學@@@@@@@@文章," +
                "Jeffrey董又霖@@@@@@@@董又霖," +
                "宋轶xiaoyiyi@@@@@@@@宋轶," +
                "李宁@@@@@@@@李宁," +
                "贾玲@@@@@@@@贾玲," +
                "workership@@@@@@@@刘慈欣," +
                "王凯kkw@@@@@@@@王凯," +
                "朱丹@@@@@@@@朱丹," +
                "BTS_official@@@@@@@@防弹少年团," +
                "薛之谦@@@@@@@@薛之谦," +
                "林彦俊@@@@@@@@林彦俊," +
                "turbosun@@@@@@@@孙俪," +
                "宋慧乔@@@@@@@@宋慧乔," +
                "久石让乐迷网@@@@@@@@久石让," +
                "GEM鄧紫棋@@@@@@@@邓紫棋," +
                "李诞@@@@@@@@李诞," +
                "胡可@@@@@@@@胡可," +
                "李荣浩@@@@@@@@李荣浩," +
                "百度金泫雅吧@@@@@@@@金泫雅," +
                "李易峰@@@@@@@@李易峰," +
                "张歆艺@@@@@@@@张歆艺," +
                "古天乐@@@@@@@@古天乐," +
                "全智贤吧@@@@@@@@全智贤," +
                "施展Alvin@@@@@@@@施展," +
                "外卖小哥金城武@@@@@@@@金城武," +
                "王鹤棣_Dylan@@@@@@@@王鹤棣," +
                "陈数@@@@@@@@陈数," +
                "本无之何为之@@@@@@@@何为," +
                "艾莉的冷笑话时间@@@@@@@@艾莉," +
                "被冤枉的赵宇@@@@@@@@赵宇," +
                "赵薇@@@@@@@@赵薇," +
                "黄子韬的吴乾@@@@@@@@黄子韬," +
                "侯明昊Neo@@@@@@@@侯明昊," +
                "张继科@@@@@@@@张继科," +
                "赵雅芝@@@@@@@@赵雅芝," +
                "朱一龙@@@@@@@@朱一龙," +
                "马苏@@@@@@@@马苏," +
                "宋威龙99@@@@@@@@宋威龙," +
                "周杰小先生@@@@@@@@周杰," +
                "金钟仁吧_KAIBar@@@@@@@@金钟仁," +
                "小沈阳@@@@@@@@小沈阳," +
                "TFBOYS-王源@@@@@@@@王源," +
                "William威廉陈伟霆@@@@@@@@陈伟霆," +
                "唐嫣@@@@@@@@唐嫣," +
                "美微郭沫若@@@@@@@@郭沫若," +
                "郭富城@@@@@@@@郭富城," +
                "火箭少女101_徐梦洁@@@@@@@@徐梦洁," +
                "卢思浩@@@@@@@@卢思浩," +
                "周扬@@@@@@@@周扬," +
                "徐锦江@@@@@@@@徐锦江," +
                "朴有天_Official@@@@@@@@朴有天," +
                "崔永元@@@@@@@@崔永元," +
                "何穗@@@@@@@@何穗," +
                "王一@@@@@@@@王一," +
                "王维的院子@@@@@@@@王维," +
                "钱正昊@@@@@@@@钱正昊," +
                "林萍在日本@@@@@@@@林萍," +
                "秋瓷炫国际影迷会@@@@@@@@秋瓷炫," +
                "SunMiracles宣美吧@@@@@@@@宣美," +
                "朴信惠@@@@@@@@朴信惠," +
                "胖子孙越@@@@@@@@孙越," +
                "沈月@@@@@@@@沈月," +
                "张译@@@@@@@@张译," +
                "章泽天@@@@@@@@章泽天," +
                "徐峥@@@@@@@@徐峥," +
                "郭敬明@@@@@@@@郭敬明," +
                "金星@@@@@@@@金星," +
                "杨绛语录@@@@@@@@杨绛," +
                "华晨宇yu@@@@@@@@华晨宇," +
                "聂远@@@@@@@@聂远," +
                "张雨绮@@@@@@@@张雨绮," +
                "葛优@@@@@@@@葛优," +
                "周润发@@@@@@@@周润发," +
                "李治廷@@@@@@@@李治廷," +
                "王千源@@@@@@@@王千源," +
                "孙楠@@@@@@@@孙楠," +
                "关晓彤@@@@@@@@关晓彤," +
                "胡彦斌@@@@@@@@胡彦斌," +
                "落落430@@@@@@@@落落," +
                "马化腾7853@@@@@@@@马化腾," +
                "谭松韵seven@@@@@@@@谭松韵," +
                "王心凌@@@@@@@@王心凌," +
                "王家卫@@@@@@@@王家卫," +
                "CoMixWaveFilms_新海诚公式@@@@@@@@新海诚," +
                "史蒂芬霍金_StephenHawking@@@@@@@@史蒂芬," +
                "陈赫@@@@@@@@陈赫," +
                "雷佳音@@@@@@@@雷佳音," +
                "炎亞綸_AaronYan@@@@@@@@炎亚纶," +
                "李晨@@@@@@@@李晨," +
                "林语堂@@@@@@@@林语堂," +
                "迪玛希Dimash@@@@@@@@迪玛希," +
                "广土亚一@@@@@@@@庄严," +
                "李栋旭@@@@@@@@李栋旭," +
                "任贤齐@@@@@@@@任贤齐," +
                "洛天依_官博@@@@@@@@洛天依," +
                "罗云熙Leo@@@@@@@@罗云熙," +
                "林青霞@@@@@@@@林青霞," +
                "白举纲@@@@@@@@白举纲," +
                "鞠婧祎@@@@@@@@鞠婧祎," +
                "杨树林团长@@@@@@@@杨树林," +
                "超短王者--徐克@@@@@@@@徐克," +
                "郑秀文Sammi@@@@@@@@郑秀文," +
                "veggieg@@@@@@@@王菲," +
                "微博周易@@@@@@@@周易," +
                "舒淇@@@@@@@@舒淇," +
                "papi酱@@@@@@@@papi酱," +
                "蒋劲夫@@@@@@@@蒋劲夫," +
                "大嘴巴DaMouth@@@@@@@@大嘴巴," +
                "刘欣CGTN@@@@@@@@刘欣," +
                "时代少年团-丁程鑫@@@@@@@@丁程鑫," +
                "郑容和89@@@@@@@@郑容和," +
                "孙燕姿@@@@@@@@孙燕姿," +
                "柯震東Kai@@@@@@@@柯震东," +
                "李敏镐@@@@@@@@李敏镐," +
                "仙居东魁杨梅@@@@@@@@杨梅," +
                "X玖少年团肖战DAYTOY@@@@@@@@肖战," +
                "陳妍希michelle@@@@@@@@陈妍希," +
                "梅艳芳微吧@@@@@@@@梅艳芳," +
                "郎朗@@@@@@@@郎朗," +
                "西柳耙的西兰花@@@@@@@@西兰花," +
                "王思聪@@@@@@@@王思聪," +
                "越香@@@@@@@@越香," +
                "张韶涵@@@@@@@@张韶涵," +
                "小辫儿张云雷@@@@@@@@张云雷," +
                "李维嘉@@@@@@@@李维嘉," +
                "王菊@@@@@@@@王菊," +
                "阿云嘎Musical@@@@@@@@阿云嘎," +
                "梁朝偉@@@@@@@@梁朝伟," +
                "杨九郎L@@@@@@@@杨九郎," +
                "苏醒AllenSu@@@@@@@@苏醒," +
                "韩庚@@@@@@@@韩庚," +
                "乔任梁@@@@@@@@乔任梁," +
                "陆琪@@@@@@@@陆琪," +
                "劉皮特@@@@@@@@皮特," +
                "蘇打綠Sodagreen@@@@@@@@苏打绿," +
                "烧伤超人阿宝@@@@@@@@阿宝," +
                "韩星排行榜@@@@@@@@韩星," +
                "陈果@@@@@@@@陈果," +
                "汪峰@@@@@@@@汪峰," +
                "杨迪@@@@@@@@杨迪," +
                "毛不易@@@@@@@@毛不易," +
                "袁咏仪靓靓@@@@@@@@袁咏仪," +
                "官鸿Kuan@@@@@@@@官鸿," +
                "倪慕斯倪承艾@@@@@@@@慕斯," +
                "蔡依林@@@@@@@@蔡依林," +
                "baekhyunee7@@@@@@@@边伯贤," +
                "陈坤@@@@@@@@陈坤," +
                "阮經天@@@@@@@@阮经天," +
                "李鍾碩@@@@@@@@李钟硕," +
                "张也变啊@@@@@@@@张也," +
                "李玉@@@@@@@@李玉," +
                "先生胡适@@@@@@@@胡适," +
                "王丽坤@@@@@@@@王丽坤," +
                "李志不BB@@@@@@@@李志," +
                "西藏昌都人韩红@@@@@@@@韩红," +
                "HERO趙又廷@@@@@@@@赵又廷," +
                "北瑞安娜@@@@@@@@余华," +
                "林心如@@@@@@@@林心如," +
                "王力宏@@@@@@@@王力宏," +
                "袁立@@@@@@@@袁立," +
                "吴奇隆@@@@@@@@吴奇隆," +
                "李佳@@@@@@@@李佳," +
                "唐艺昕@@@@@@@@唐艺昕," +
                "何洁@@@@@@@@何洁," +
                "高圆圆@@@@@@@@高圆圆," +
                "KangDaniel_姜丹尼尔@@@@@@@@姜丹尼尔," +
                "窦骁@@@@@@@@窦骁," +
                "马东@@@@@@@@马东," +
                "金钟@@@@@@@@金钟," +
                "刘亦菲@@@@@@@@刘亦菲," +
                "阿拉蕾雅涵@@@@@@@@阿拉蕾," +
                "赖冠霖_Edward@@@@@@@@赖冠霖," +
                "演员胡先煦@@@@@@@@胡先煦," +
                "Baidu田柾国吧@@@@@@@@田柾国," +
                "六小龄童@@@@@@@@六小龄童," +
                "张哲瀚疯子@@@@@@@@张哲瀚," +
                "小鬼-王琳凯@@@@@@@@王琳凯," +
                "程氏针灸_陶冶@@@@@@@@陶冶," +
                "江澄@@@@@@@@江澄," +
                "周渝民@@@@@@@@周渝民," +
                "王毅老师@@@@@@@@王毅," +
                "马伊琍@@@@@@@@马伊琍," +
                "阚清子@@@@@@@@阚清子," +
                "范丞丞Adam0616@@@@@@@@范丞丞," +
                "王晰@@@@@@@@王晰," +
                "張敬軒Official@@@@@@@@张敬轩," +
                "heybiblee_official@@@@@@@@李圣经," +
                "美食作家王刚@@@@@@@@王刚," +
                "梁家辉@@@@@@@@梁家辉," +
                "演员楠哥@@@@@@@@李楠," +
                "屈楚萧@@@@@@@@屈楚萧," +
                "杀马特强子@@@@@@@@马特," +
                "张超Juck@@@@@@@@张超," +
                "咪蒙的微故事@@@@@@@@咪蒙," +
                "吴昕@@@@@@@@吴昕," +
                "R1SE-刘也@@@@@@@@刘也," +
                "郁可唯@@@@@@@@郁可唯," +
                "郑爽微博事务部@@@@@@@@郑爽," +
                "八月长安就是二熊@@@@@@@@八月长安," +
                "任達華@@@@@@@@任达华," +
                "MD_摩登兄弟@@@@@@@@摩登兄弟," +
                "-张飞飞-@@@@@@@@张飞," +
                "易建联@@@@@@@@易建联," +
                "郭碧婷@@@@@@@@郭碧婷," +
                "李小龙Bruce_Lee@@@@@@@@李小龙," +
                "许晴随行笔记@@@@@@@@许晴," +
                "演员乔杉@@@@@@@@乔杉," +
                "蕭敬騰-獅子LION@@@@@@@@萧敬腾," +
                "姚晨@@@@@@@@姚晨," +
                "吳彥祖@@@@@@@@吴彦祖," +
                "黄磊微博@@@@@@@@黄磊," +
                "i周迅@@@@@@@@周迅," +
                "俞灏明@@@@@@@@俞灏明," +
                "冯绍峰@@@@@@@@冯绍峰," +
                "霍尊@@@@@@@@霍尊," +
                "雷军@@@@@@@@雷军," +
                "谋影世界之张艺谋@@@@@@@@张艺谋," +
                "李现ing@@@@@@@@李现," +
                "如是我闻_一时_@@@@@@@@朴树," +
                "朱茵@@@@@@@@朱茵," +
                "阿尔法小分队科教组@@@@@@@@阿尔法," +
                "NeymarJr@@@@@@@@内马尔," +
                "火箭少女101_李紫婷@@@@@@@@李紫婷," +
                "張宇@@@@@@@@张宇," +
                "張震嶽@@@@@@@@张震岳," +
                "吴倩mine4ever@@@@@@@@吴倩," +
                "于谦@@@@@@@@于谦," +
                "张琼瑶Emma@@@@@@@@琼瑶," +
                "胡军@@@@@@@@胡军," +
                "张恒@@@@@@@@张恒," +
                "汪涵@@@@@@@@汪涵," +
                "刘维-Julius@@@@@@@@刘维," +
                "木村拓哉@@@@@@@@木村拓哉," +
                "安康警务@@@@@@@@安康," +
                "陈飞宇Arthur@@@@@@@@陈飞宇," +
                "韩寒@@@@@@@@韩寒," +
                "秦岚@@@@@@@@秦岚," +
                "張震@@@@@@@@张震," +
                "火箭少女101_赖美云@@@@@@@@赖美云," +
                "尤长靖@@@@@@@@尤长靖," +
                "陳小春@@@@@@@@陈小春," +
                "甄子丹@@@@@@@@甄子丹," +
                "演员马丽@@@@@@@@马丽," +
                "韩雪@@@@@@@@韩雪," +
                "宋小宝纯行货@@@@@@@@宋小宝," +
                "王鸥Angel@@@@@@@@王鸥," +
                "任素汐@@@@@@@@任素汐," +
                "锋味@@@@@@@@谢霆锋";
        File file = new File("name-mapping.csv");
        if (!file.exists()) {
            try {
                file.createNewFile();
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        Appendable printWriter = new PrintWriter(file,"utf-8");//指定GBK,解决Microsoft不兼容
        CSVPrinter csvPrinter = CSVFormat.EXCEL.withHeader("wbName", "realName").print(printWriter);
        for (String s : name.split(",")) {
            String[] split = s.split("@@@@@@@@");
            csvPrinter.printRecord(split[0], split[1]);
        }
        csvPrinter.flush();
        csvPrinter.close();
    }

    private void printLoadLog(String fileName, String msg) {
        File file = new File("config\\" + fileName);
        FileOutputStream fileOutputStream = null;
        try {
            fileOutputStream = new FileOutputStream(file, true);
            byte[] contentInBytes = (msg + "\r\n").getBytes();
            fileOutputStream.write(contentInBytes);
            fileOutputStream.flush();
            fileOutputStream.close();
        } catch (IOException e) {
            e.printStackTrace();
        } finally {
            try {
                if (fileOutputStream != null) {
                    fileOutputStream.close();
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
    }

}
